#!/usr/bin/env bash
# Train and evaluate one local prior/reconstruction experiment.
#
# Usage:
#   scripts/run_eeg_prior.sh [Hydra overrides...]
#
# Example:
#   scripts/run_eeg_prior.sh model.max_epochs=1 trainer.wandb.enabled=false
...
experiment_root="${EEG_PRIOR_OUTPUT_ROOT:-experiments/eeg_prior}"
run_group="${experiment_root}/$(date +%Y%m%d_%H%M%S)"
mkdir -p "$run_group"

echo "Training EEG prior with config train_eeg_prior"
echo "Output root: $run_group"

python scripts/training/train_eeg_prior.py \
    --config-name=train_eeg_prior \

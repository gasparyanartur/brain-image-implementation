...
from brain_image.data_preprocessing import generate_preprocessed_dataset
...
def _download_to_file(
    ...
    if total_size > 0 and (written_size != total_size):
        raise ValueError(f"Downloaded size does not match expected size: {written_size} != {total_size}")
...
def main(args: Args):
    ...
    if args.get_img:
        ...
        extracted_path = args.data_path / args.img_dir
        training_img_path_zip = extracted_path / "training_images.zip"
        test_img_path_zip = extracted_path / "test_images.zip"

...
def main(args: Args):
    ...
    model = CommAlignmentModel.load_from_checkpoint(args.checkpoint_path, hparams_file=args.hyperparameters_path)
    model.eval()
    ...
    for key in mean_metrics.keys():
        logging.info(f"  {key}: {mean_metrics[key].item()} ± {std_metrics[key].item()} ")

...
def epoching(args: ThingsEEG2DatasetPreprocessingConfig, data_part, seed):
	...
	for s in range(args.n_ses):
		...
		eeg_data = np.load(raw_eeg_path, allow_pickle=True).item()
		...
		epochs = mne.Epochs(
			raw, events, tmin=-0.2, tmax=1.0, baseline=(None, 0), preload=True
		)
		del raw
...
def mvnn(args: ThingsEEG2DatasetPreprocessingConfig, epoched_test, epoched_train):
...
def save_prepr(
	args: ThingsEEG2DatasetPreprocessingConfig,
	whitened_test,
	whitened_train,
